...
=head1 SYNOPSIS
    ...
    my $q = Politequeue->new(db => ":memory:");

    # Put a message in the queue
    my $message = $q->put("example task");

    # Pop a message from the queue
    my $popped = $q->pop();
    if ($popped) {
        print "Processing: " . $popped->data;
        # ... process message ...
        $q->done($popped->message_id);
    }
...
Much of this documentation and the core function is a direct port of litequeue from Python, to ensure compatibility and future updates.
...
If you specify a C<maxsize> when you initialize the queue, it will create a trigger that will raise an error when that size is reached. In Perl, it will raise a DBI exception.
    ...
    my $q = Politequeue->new(db => ":memory:", maxsize => 50);
...
When we C<pop> an item we can add another one. Take into account that C<$q-E<gt>put()> will return the C<rowid> of the latest inserted message, it does B<not> represent the current queue size.
...
=head2 new
    ...
    my $q = Politequeue->new(db => $file_or_dbh, %options);

    my $q = Politequeue->new($file_path);

    my $q = Politequeue->new(':memory:');

Creates a new Politequeue object. Provide key-value pairs or pass a single argument to use as the database filename or existing DBI database handle.

B<Arguments:>

=over 4

=item * C<db> - Database filename, ":memory:" for in-memory database, or existing DBI database handle

=item * C<filename_or_conn> - Alias for C<db> for consistency with the original Python API
...
Inserts a new message into the queue. If the queue has C<maxsize> and the number of STATUS_READY rows exceeds C<maxsize> an exception will be thrown.
...
Checks if the queue is full. Always returns false if no C<maxsize> is set.

B<Arguments:> None

B<Returns:> Boolean (1 if full, 0 if not full or no C<maxsize> set)
